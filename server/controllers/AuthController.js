...
const forgotPassword = async (req, res, next) => {
  ...
  try {
    ...
    if(!employee) return res.status(404).json({ success: false, message: 'Email address not found' });
...
const resetPassword = async (req, res, next) => {
  ...
  try {
    ...
    if(!employee) return res.status(400).json({ success: false, error: "Invalid Reset Token" });
