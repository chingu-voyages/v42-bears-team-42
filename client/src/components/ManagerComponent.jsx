...
export default function ManagerComponent() {
  ...
  const tabStyle =
    "w-1/2 py-2 text-center rounded-lg border-2 border-purple-700 font-semibold";
  ...
  return (
    <div className="w-full h-full flex flex-col bg-gray-200">
      <div className="tabRow flex flex-row bg-purple-700">
        <div
          onClick={(e) => setActiveTab("scheduler")}
          className={`${tabStyle} ${activeScheduler}`}
        >
          Scheduler
        </div>
        <div
          onClick={(e) => setActiveTab("employee")}
          className={`${tabStyle} ${activeEmployee}`}
        >
          Employee Management
        </div>
      </div>
      <div className="tabContent h-full p-1 text-center">
